    ...
    async (file: File, id: string, tries = 0): Promise<void> => {
      ...
      if (result.status === "error") {
        if (autoRetry === true && tries < (maxRetryCount ?? Infinity)) {
          ...
          // Use a recursive call via a local helper to avoid ESLint hook ordering issues
          const retry = async (t: number): Promise<void> => {
            const r = await pOnDropFile(file);
            if (r.status === "error") {
              if (autoRetry === true && t < (maxRetryCount ?? Infinity)) {
                dispatch({ type: "update-status", id, status: "pending" });
                return retry(t + 1);
              }
              dispatch({
                ...
                error:
                  pShapeUploadError !== undefined
                    ? pShapeUploadError(r.error)
                    : r.error,
              });
              if (pOnFileUploadError !== undefined) {
                pOnFileUploadError(r.error);
              }
              return;
            }
            if (pOnFileUploaded !== undefined) {
              pOnFileUploaded(r.result);
            }
            dispatch({ type: "update-status", id, ...r });
          };
  ...
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      ...
      <div
        ...
        aria-label="dropzone"
        className={cn(
          "border-input bg-background ring-offset-background focus-visible:ring-ring flex items-center justify-center border border-dashed px-4 py-2 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none",
  ...
  return (
    ...
    >
      <li
        ...
        className={cn(
          "flex flex-col justify-center gap-2 border px-2 py-2",
          className,
        )}
      >
        {props.children}
...
type DropzoneRemoveFileProps = React.ComponentProps<typeof Button>;
...
type DropzoneRetryFileProps = React.ComponentProps<typeof Button>;
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <label
        ...
        {...props}
        className={cn(
          "bg-secondary ring-offset-background hover:bg-secondary/80 has-[input:focus-visible]:ring-ring cursor-pointer px-4 py-2 font-medium transition-colors focus-within:outline-none has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-offset-2",
          className,
        )}
      >
        {children}
